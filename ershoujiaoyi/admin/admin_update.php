...
$res=$a->getonedata('news_table',$_GET['update'],MYSQL_ASSOC);
	$htmlData = '';
...
<body>
	<div>
		<?php echo $htmlData; ?>
		<form action="admin_update_ok.php?id=<?php echo $res['id']?>" name="example" method="post" enctype="multipart/form-data">
			标题:<input type="text" name="title" value="<?php echo $res['title']?>"><br/>
			类型:<select name="la">
			...
			作者:<input type="text" name="writer" <?php if(!empty($rem['id'])){?> value="<?php echo $rem['write']?>" <?php }?>><br/>
			
			内容:<textarea name="content" style="width:700px;height:200px;visibility:hidden;"><?php echo htmlspecialchars($htmlData);?><?php echo $res['content']?></textarea>
				<br />
				<!-- htmlspecialchars($htmlData) -->
			浏览量:<input type="text" name="hit" value="<?php if(!empty($rem['id'])){?> value="<?php echo $rem['hit']?>" <?php }?>"><br/>	
